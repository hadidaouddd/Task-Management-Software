...
const EditTask = ({
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async () => {
      return axios.put(`http://localhost:8080/api/tasks/${selectedTask.id}`, {
        title,
        description,
    ...
    onSuccess: () => {
      ...
      setDescription("");
      setSubtasks([""]);
      setSelectedColumnId(fillcolumns[0]?.id ?? "");
    },
  });
  ...
  useEffect(() => {
    if (selectedTask && openTask) {
      setTitle(selectedTask.title || "");
      setDescription(selectedTask.description || "");
      setSubtasks(selectedTask.subtasks?.map((st) => st.title) || [""]);
      setSelectedColumnId(selectedTask.column_id || "");
    }
